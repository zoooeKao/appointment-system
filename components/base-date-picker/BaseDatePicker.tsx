...
import type { BaseDatePickerProps, TimeSlotType, TimeType } from './type';
...
const BaseDatePicker: React.FC<BaseDatePickerProps> = ({
  ...
  const handleFocus =
    (slot: TimeSlotType): FocusEventHandler<HTMLElement> =>
    e => {
      try {
        ...
        setTimeout(() => {
          ...
          if (list) {
            list.scrollTop = slot === 'morning' ? 0 : list.scrollHeight / 2;
          }
        }, 0);
      } catch (e) {
        console.error(e);
      ...
      <section className="flex items-center gap-2">
        ...
        <div>
          <DatePicker
            ...
            onFocus={handleFocus(timeSlot[0])}
          />
        </div>
      ...
      <section className="flex items-center gap-2">
        ...
        <div>
          <DatePicker
            ...
            placeholderText={isScheduleOpen() ? '開始' : '-'}
            className={datePickerClassName}
            onFocus={handleFocus(timeSlot[1])}
            disabled={isUploading || !isScheduleOpen()}
          />
        </div>
